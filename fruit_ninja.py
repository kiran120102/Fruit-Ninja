...
fruits = [ 'dragon', 'mango', 'strawberry', 'apple', 'jackfruit', 'kiwi', 'bomb']    #entities in the game
...
background = pygame.image.load('abhibg.png')                                  #game background
...
def show_gameover_screen():
    ...
    player_lives = 1
    draw_lives(gameDisplay, WIDTH / 5.5, HEIGHT / 800 / 801, player_lives, 'images/abfruit.png')

    player_lives = 1
    draw_lives(gameDisplay, WIDTH / 2.8, HEIGHT * 3 / 4, player_lives, 'images/dragon.png')

    if not game_over :
        draw_text(gameDisplay,"SCORE : " + str(score), 70, WIDTH / 2, HEIGHT /1.8)

        draw_text(gameDisplay, "  again!", 60, WIDTH / 1.2, HEIGHT * 3 / 4)

    draw_text(gameDisplay, "click on       fruit to begin", 60, WIDTH / 2.2, HEIGHT * 3 / 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
